...
+ 컴퓨터끼리 연결되는 네트워크 세계에서 주소가 있어야 어느 컴퓨터에 연결할지  확인 가능
+ IP(Internet Protocol address)는 네트워크 통신을 위한 주소
+ 어떤 네트워크 사용하는지에 따라 IP가 변경됨
+ 예를 들어 '203.252.143.23' 처럼 4개의 공간에 0~255의 값이 붙어 있는 형식이다. 이를 IPv4라고 한다. 최근에는 인터넷에 접속할 수 있는 기기가 기하급수적으로 늘어나면서 4개의 공간만으로는 인터넷 주소의 수요를 맞출 수 없어 IPv6라는 인터넷 주소 체계를 사용하기 시작했다.



#### 2. 포트 번호

* 네트워크 IP를 통해 컴퓨터와 연결된 후 컴퓨터 안에 실행되고 있는 프로그램에 최종적으로 도달하기 위해선 포트번호가 필요함



#### 3. 도메인 네임

* IP와 포트번호는 숫자로만 이루어져 있어 사용하기가 어렵다. 그래서 사람이 쓰기 쉽도록 영문 주소로 만든 것이 도메인 네임이다.
* 도메인 네임 서버(DNS)는 도메인 네임과 IP, 포트번호를 가진 서버로서 우리가 도메인 네임을 입력하면 IP와 포트번호로 해당 서버에 접속할 수 있도록 도와준다



#### 4. 웹의 동작

* 그렇다면 요청한다는 것은 무엇을 뜻할까? 간단히 말해 내가 가진 주소의 정보를 내 컴퓨터로 다운로드하게 해달라는 것이다. 여기서 정보는 HTML, CSS, 자바스크립트와 같이 웹 페이지를 꾸미는 기능을 하는 코드로 다운로드하게 된다. 또한, 동영상이나 그림 같은 리소스도 다운로드한다. 이렇게 다운로드한 정보를 내 컴퓨터에서 시작적으로 표현하기 위해 컴퓨터의 웹 브라우저는 그 정보를 해석하고 렌더링을 시행한다. 렌더링은 어떤 정보나 코드를 사용하여 시각적으로 어떤 약속에 기반하여 표현하는 과정이다. 예를 들어, 우리가 스마트폰으로 영상을 찍으면 디지털 파일로 저장되는데, 이를 mp3와 같은 다른 형태로 변경할 때 렌더링을 다시한다고  말한다. 웹 브라우저에서 `F12`를 누르면 다운로드하는 정보와 렌더링하여 표현되는 정보를 볼 수 있다.
...
* 예전에는 휴대폰 앞에 010, 011, 016, 018, 019 등이 있었다. 이 경우 01은 반드시 포함되고 뒤에 어떤 숫자가 한번만 반복될 경우 메타문자 물음표 ? 를 사용할 수 있다. 물음표?는 0,1번만 반복될 경우 사용한다
